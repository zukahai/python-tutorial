...
class WaveManager:
    ...
    def spawn_wave(self, enemies):
        for _ in range(self.wave * 5):  # Tạo nhiều kẻ thù hơn mỗi làn sóng
            enemy = Player(random.choice(WAYPOINTS)[0], random.choice(WAYPOINTS)[1], self.wave)
            enemies.add(enemy)
        self.wave += 1
